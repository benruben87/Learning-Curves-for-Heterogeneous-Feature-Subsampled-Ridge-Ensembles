...
def runEvP_Class_Ensemble(expt_name, x, y, ConnDist, ensFunc, sigma_0, eta_r_scale, projType, learningRule, num_samples_list, KVals, lamVals, num_trials, shuffle_degrees):
...
#   test_acc = np.empty([numPVals, numKVals, num_trials])
    ...
    tr_err = np.empty([numKVals, numPVals, numLamVals, num_trials])
    test_err = np.empty([numKVals, numPVals, numLamVals, num_trials])
...
#Learning Curves for Different Distriutons of Connectivity.
def runEvP_MultiClass_CompDists(x, y, ConnDists, ensFunc, sigma_0, eta_r_scale, projType, learningRule, num_samples_list, KVals, lamVals, num_trials, shuffle_degrees, downFrac = 1, log_file="experiment_log.txt"):
    
    #Downsampling fraction is the fraction of features that are used in any experiment.
...
#   test_acc = np.empty([numPVals, numKVals, num_trials])
    ...
    ns = EnsembleLib.genNs(KVals[0], M, ConnDists[0]) #Placeholder
    student = EnsembleLib.MultiReadout(M, C, ns, sigma_0, eta_r_scale, auxFuncs.oneHotArgMax, align_proj = align_proj)
    
    for KInd, K in enumerate(KVals):
        for ConnDistInd, ConnDist in enumerate(ConnDists):
            ns = EnsembleLib.genNs(K, downFrac*M, ConnDist)
            print(ns)
            student.setDegrees(ns)
            for PInd, P in enumerate(num_samples_list):
                start_time = datetime.now(tz)
                for trial in range(num_trials):
                    ...
                    for lamInd, lam in enumerate(lamVals):
                        student.regress_readout(train_loader, test_loader, lam)
                        errors = student.eval(train_loader, test_loader, ensFunc, auxFuncs.MultiClassErrorRate)
                        tr_err[KInd, ConnDistInd, PInd, lamInd, trial] = errors[0]
                        test_err[KInd, ConnDistInd, PInd, lamInd, trial] = errors[1]
                            
                 # Log the message with a timestamp
                log_message = f'K = {K}; ConnDist = {ConnDist}; P = {P}; Eg = {np.mean(test_err[KInd, ConnDistInd, PInd, :, :])}; time: {time_diff(start_time, datetime.now(tz))}'
...
def runEvP_LinFixedMasks(expt_name, sigma_s, sigma_0, ns, eta, zeta, lams, num_samples_list, num_trials, exclusive = False, aveGTW = False, rho = -2):
    ...
    student = EnsembleLib.MultiReadout(M, 1, ns, sigma_0, eta, auxFuncs.identity, align_proj = True, exclusive = exclusive)
